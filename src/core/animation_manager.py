...
class AnimationManager:
    ...
    def update_frame(self):
        ...
        if self.current_state == "idle":
            ...
            # Прыжки (пропорционально высоте окна)
            jump_h = size.height() * 0.15
            painter.translate(0, -abs(jump_h * math.sin(self.frame_counter * 0.5)))
            ...
            # Наклоны головы вперед-назад при еде (пропорционально высоте окна)
            scale_y = 1.0 + 0.1 * abs(math.sin(self.frame_counter * 0.8))
            eating_offset = size.height() * 0.1 * (scale_y - 1.0)
            painter.translate(0, eating_offset)
