...
class AnimationManager:
    ...
    def update_frame(self):
        ...
        if self.current_state == "idle":
            ...
            # Прыжки (динамически масштабируемые пропорционально высоте)
            ...
            # Наклоны головы вперед-назад при еде (динамически масштабируемые пропорционально высоте)
            scale_y = 1.0 + 0.1 * abs(math.sin(self.frame_counter * 0.8))
            eat_offset = size.height() * 0.1 * (scale_y - 1.0)
