...
class AnimationManager:
    ...
    def update_frame(self):
        ...
        if self.current_state == "idle":
            ...
            # Прыжки (высота прыжка пропорциональна высоте окна/размеру)
            jump_height = max(10, size.height() * 0.15)
            ...
            # Наклоны головы вперед-назад при еде (смещение пропорционально высоте окна)
            shift_y = max(5, size.height() * 0.10)
            scale_y = 1.0 + 0.1 * abs(math.sin(self.frame_counter * 0.8))
            painter.translate(0, shift_y * (scale_y - 1.0))
