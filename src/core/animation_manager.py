...
class AnimationManager:
    ...
    def update_frame(self):
        ...
        if self.current_state == "idle":
            ...
            # Прыжки (высота прыжка пропорциональна высоте окна)
            ...
            # Наклоны головы вперед-назад при еде (смещение пропорционально высоте)
            scale_y = 1.0 + 0.1 * abs(math.sin(self.frame_counter * 0.8))
            eat_shift = size.height() * 0.1
            painter.translate(0, eat_shift * (scale_y - 1.0))
