...
class InputManager(QObject):
    ...
    def handle_mouse(self, x, y):
        ...
        # Динамический порог радиуса поглаживания на основе размера окна (0.6 от минимального измерения)
        pet_radius = min(self.window.width(), self.window.height()) * 0.6
        threshold_sq_pet = pet_radius * pet_radius

        # Оптимизация: сравнение квадрата расстояния с динамическим порогом
        if dist_sq_pet < threshold_sq_pet:
